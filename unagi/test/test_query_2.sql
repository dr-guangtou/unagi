...
--   --> Edit the schema name 's18a_wide' for your query.
...
SELECT
    ...
    FROM
      ...
      LEFT JOIN
        s18a_wide.forced2 AS f2 USING (object_id)
      LEFT JOIN
        s18a_wide.meas   AS m1 USING (object_id)
      LEFT JOIN
        s18a_wide.meas2  AS m2 USING (object_id)
    WHERE
        f1.isprimary
